...
class Point(object):
    ...
    def __repr__(self):
        """ Return a string that represents the point object. """
        return 'Point(' + str(self.x) + ', ' + str(self.y) + ')'
        # return "Point({}, {})".format(self.s,self.y)
    ...
    def move_to(self, x, y):
        self.distanceTraveled += ((self.x - x)**2 + (self.y - y)**2) ** .5
        self.x = x
        self.y = y
        self.moveCounter += 1

    def move_by(self, dx, dy):
        self.distanceTraveled += (dx**2 + dy**2)**.5
        self.x += dx
        self.y += dy
        self.moveCounter += 1
    ...
    def get_distance_from_start(self):
        return round(((self.initXPos - self.x)**2 + (self.initYPos - self.y)**2) ** .5, 3)
